...
if ($edittype == 'username') {
    ...
    $sql = "UPDATE doctortbl SET `$edittype` = '$newtext' WHERE doctorID = $userID";
    mysqli_query($conn, $sql);
    $_SESSION['user'] = mysqli_query($conn, "SELECT * FROM doctortbl WHERE doctorID =" . $userID)->fetch_assoc();
}
