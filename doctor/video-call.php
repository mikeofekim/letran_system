...
<body>
    <div class="bg-white p-3 shadow-sm mb-3">
        <div class="container">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex ">
                    <div class="h6 mb-0 text-secondary">Dr. <?= $display_name ?></div>
                </div>
    ...
    <div class="container">
        <div class="mb-3">
            <button type="button" class="btn btn-danger w-100 shadow" onclick="endCall()"><i class="fas fa-phone-slash"></i> End Consultation Session</button>
        </div>
    ...
    <script>
        ...
        api.addEventListener('participantJoined', function(event) {
            Swal.fire({
                position: 'top-end',
                icon: 'info',
                title: event.displayName + 'joined!',
        ...
        function endCall() {
            if (confirm("Are you sure?")) {
                ...
                $.post("ajax/sendmessage.php", {
                    conid: '<?= $vcdata['consultationID'] ?>',
                    doc: '<?= $vcdata['patientID'] ?>',
                    notif: '<i class="fas fa-video-slash"></i> Meeting ended.'
                }, function(data) {

                });

                $.post('ajax/endcall.php', {
                    vcid: '<?= $vcdata['vcid'] ?>'
                }, function() {
                    window.close();
                });
